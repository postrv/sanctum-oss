...
///
/// Prints useful offline diagnostics, then returns `Err(CliError::DaemonNotRunning)`
/// so that shell hooks (`if ! sanctum daemon status`) can detect offline state
/// and auto-start the daemon.
