...
pub fn write_ca_files(
    ...
    if let Some(parent) = cert_path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| ProxyError::CaKeyFile { source: e })?;
    }
...
mod tests {
    ...
    fn test_generate_site_cert_multiple_domains() {
        ...
        for domain in domains {
            let result = generate_site_cert(&ca, domain);
    ...
    fn test_default_paths() {
        ...
        let cert_path = default_cert_path(data_dir);
    ...
    fn test_save_creates_parent_directories() {
        ...
        let ca = generate_ca(365).expect("generate CA");
        let result = write_ca_files(&ca, &cert_path, &key_path);
